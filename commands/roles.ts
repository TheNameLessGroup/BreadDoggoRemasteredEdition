...
export default {
    ...
    callback: async ({ message, interaction, args, client }) => {
        ...
        if (!channel || channel.type !== 'GUILD_TEXT') {
            return 'Please tag a channel owo'
        }
        ...
        if (!targetMessage) {
            return 'Unknown message id oof'
        }

        if (targetMessage.author.id !== client.user?.id) {
            return `Please provide a message ID that was sent form <@${client.user?.id}>`
        }
        ...
        if (menu) {
            for (const o of menu.options) {
                if (o.value === option[0].value) {
                    return {
                        custom: true,
                        content: `<@&${o.value}> is already part of this menu -w-`,
                        allowedMentions: {
                            roles: []
                        },
                        ephemeral: true
            ...
            row.addComponents(
                new MessageSelectMenu()
                    .setCustomId('auto_roles') // This value has to be different every time
                    .setMinValues(0)
                    .setMaxValues(1)
